extends Node
## Headless checks for CollisionSystem: ramming a solid body damages the hull,
## logs a COLLISION event, and stops the ship at the surface (no tunnelling);
...
func _run() -> void:
	...
	# On site with a wreck at DEFAULT_WRECK_POS (no 3D scene loaded headless).
	...
	var ship_radius: float = CollisionScript.SHIP_RADIUS
	var ram_min_sep: float = ship_radius + CollisionScript.WRECK_RADIUS

	# --- Gentle nudge below the speed floor: separate, no damage ---------------
	var buoy_pos := Vector3(100, 0, 0)
	var buoy_id := GameState.register_obstacle("TEST BUOY", buoy_pos, 3.0)
	var buoy_min_sep: float = ship_radius + 3.0
	var hull_before_gentle := _hull_total()
	var ship: Dictionary = GameState.local_ship()
	# Sit just inside the buoy with a slow inward drift (well under the floor).
	var into_buoy := (buoy_pos - Vector3(120, 0, 0)).normalized()  # points -X, toward buoy
	ship["transform"] = Transform3D(Basis.IDENTITY, buoy_pos - into_buoy * (buoy_min_sep - 0.2))
	ship["velocity"] = into_buoy * 0.5
	await _wait(0.5)
	_check(not _has_comms("TEST BUOY"), "gentle contact logs no collision")
	_check(is_equal_approx_soft(_hull_total(), hull_before_gentle),
			"gentle contact deals no hull damage")
	_check(GameState.local_ship()["transform"].origin.distance_to(buoy_pos)
			>= buoy_min_sep - 0.3, "gentle contact still separates the ship")
	GameState.remove_obstacle(buoy_id)

	# --- Hard ram into the wreck: damage + stop at the surface -----------------
	# Reset the ship to the origin, facing the wreck (-Z), at rest.
	...
	var rammed := await _wait_until(func() -> bool: return _has_comms_since(comms_before, "COLLISION"), 12.0)
	...
	await _wait(0.3)
	var dist_after: float = ship["transform"].origin.distance_to(wreck_pos)
	_check(dist_after > ram_min_sep - 1.0,
			"ship stopped near the surface, no tunnelling (dist %.1f, sep %.1f)" % [
				dist_after, ram_min_sep])
	_check(ship["transform"].origin.z > wreck_pos.z,
			"ship did not pass through the wreck")
